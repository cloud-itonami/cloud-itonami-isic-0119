(ns othercropops.phase
  "Phase gate for the other-non-perennial-crop-growing operations
  coordinator rollout (0->3 maturity). Mirrors `vegops.phase`
  (cloud-itonami-isic-0113) in shape, adapted to this actor's simpler op
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`othercropops.governor/always-escalate-ops`) escalate even when
  ...
  (:require [othercropops.governor :as governor]))
