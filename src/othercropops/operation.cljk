(ns othercropops.operation
  "OperationActor -- one other-non-perennial-crop-growing operation = one
  supervised actor run. The advisor (OtherCropOpsAdvisor) is sealed into a
  ...
  One run = one other-non-perennial-crop-growing coordination operation
  ...
  is clean -- see `othercropops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `vegops.operation`, cloud-itonami-isic-0113). This stub version defines
  the high-level flow synchronously; production build wires this into a
  langgraph-clj StateGraph with `interrupt-before` for the escalation node
  and checkpoint-based resume."
  (:require [othercropops.advisor :as advisor]
            [othercropops.governor :as governor]
            [othercropops.phase :as phase]))
...
(defn run-operation
  "Run one other-non-perennial-crop-growing operation through the advisor
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- an `othercropops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
